...
void setup() {
 ...
 prizm.setServoPosition(1,175.5);       // set the initial position of the gripper
 prizm.setServoPosition(2,90);          //set the initial position of the arm
 ps4.setRumble(STOP);
}
...
void loop() {
  ...
  if (ps4.Button(CROSS)==1){prizm.setServoPosition(2, 75.5);delay(100);}                            // if the CROSS button is pressed,lower the arm
  if (ps4.Button(SQUARE)==1){prizm.setServoPosition(2, 90);delay(100);}                           // if the SQUARE button is pressed, stop the motion of the arm 
  if (ps4.Button(TRIANGLE)==1){prizm.setServoPosition(2, 105);delay(100);}                          // if the TRIANGLE button is pressed, raise arm
  if (ps4.Button(CIRCLE)==1){prizm.setServoPosition(2, 90);delay(100);}                             // if the CIRCLE button is pressed, stop the motion of the arm
  prizm.resetEncoders();

  if (ps4.buttons_2 == 8){prizm.setServoPosition(1, 80);}                               //if the left cross is pressed, open the gripper
  if (ps4.buttons_2 == 4){prizm.setServoPosition(1, 175.5);}                            //if the right cross is pressed, close the gripper
}                                                                                       // end the main loop
